...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'NIM Janitor RP Proxy (Deepseek-v3.2) + Streaming' });
});

/* ============================================================
   HELPER – build the final messages array
...
async function buildFinalMessages(reqBody) {
  ...
  const genModel = MODEL_MAPPING[model] || MODEL_MAPPING['deepseek-3.2'] || 'deepseek-ai/deepseek-v3.2';
  ...
  const CHAT_ID = reqBody.chat_id || `temp-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  ...
  sysParts.push({
    role: 'system',
    content: `You are a fictional character in an ongoing roleplay. Stay fully in character at all times. Use dialogue and descriptive actions (*like this*). Never mention AI or systems. Avoid short replies. Continue the scene naturally. You will never talk for {{user}}. If other characters are present, you will speak and act for all of them.`
  });
